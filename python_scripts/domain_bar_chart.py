...
# Read the CSV files
df_pd1 = pd.read_csv('pd2cputotal_power.csv', header=None, names=['Power Consumption'])
df_pd2 = pd.read_csv('pd2memtotal_power.csv', header=None, names=['Power Consumption'])
...
plt.bar(index, total_pd1_power, bar_width, label='CPU', color='green')
...
plt.legend()
plt.tight_layout()
plt.savefig('pd2power_consumption_stacked_bar_chart.png')
plt.show()
